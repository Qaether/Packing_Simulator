...
def detect_o_motifs(
    ...
    motifs = set()
    triangle_set = set(tuple(sorted(t)) for t in triangles) if triangles is not None else set()
    square_set = set(tuple(sorted(s)) for s in squares) if squares is not None else set()

    for square in square_set:
        pole_candidates = set(graph.neighbors(square[0]))
        for vertex in square[1:]:
            pole_candidates &= set(graph.neighbors(vertex))

        for pole_a, pole_b in combinations(pole_candidates, 2):
            if graph.has_edge(pole_a, pole_b):
                continue
            nodes = tuple(sorted((*square, pole_a, pole_b)))
            if nodes in motifs:
                continue
            ...
            points = centered_points(nodes, state.positions, state.box)
            node_pos = {node: points[idx] for idx, node in enumerate(nodes)}
            centers = []
            axes = []
            for a, b in pair_triple:
                ...
                if triangle_set:
                    motif_triangles = [
                        tuple(sorted(t))
                        for t in combinations(nodes, 3)
                        if tuple(sorted(t)) in triangle_set
                    ]
                ...
                motif_squares = [
                    ...
                    if tuple(sorted(s)) in square_set
                ]
                if len(motif_squares) < 3:
                    continue
                square_edge_counts = Counter()
                used_square_count = 0
                for motif_square in motif_squares:
                    cycle_edges = _square_cycle_edges(motif_square, sub)
